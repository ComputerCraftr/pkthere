        ...
        let worker_flows: Vec<_> = sock_mgrs
            ...
            .map(|(worker_pair, (sock_mgr, state))| {
                ...
                json!({
                    ...
                    "client_connected": if locked { snapshot.listener_connected } else { false },
                    ...
                    "client_canonical": if locked { snapshot.client_remote.map(|addr| addr.to_string()) } else { None::<String> },
                    "client_remote_canonical": if locked { snapshot.client_remote.map(|addr| addr.to_string()) } else { None::<String> },
                    "listen_canonical": snapshot.listen_local_filter.to_string(),
                    "listen_filter_canonical": snapshot.listen_local_filter.to_string(),
                    "listen_kernel_canonical": snapshot.listen_local_kernel.to_string(),
                    "listen_local_filter_canonical": snapshot.listen_local_filter.to_string(),
                    "listen_local_kernel_canonical": snapshot.listen_local_kernel.to_string(),
                    "upstream_canonical": snapshot.upstream_remote_filter.to_string(),
                    "upstream_filter_canonical": snapshot.upstream_remote_filter.to_string(),
                    "upstream_remote_filter_canonical": snapshot.upstream_remote_filter.to_string(),
                    "upstream_connected": snapshot.upstream_connected,
                    "upstream_local_canonical": snapshot.upstream_local_filter.to_string(),
                    "upstream_local_filter_canonical": snapshot.upstream_local_filter.to_string(),
                    "upstream_local_kernel_canonical": snapshot.upstream_local_kernel.to_string(),
