...
fn unconnected_client_flow_publishes_metadata_without_socket_association() {
    ...
    let published = mgr
        .establish_client_flow(key, SocketLegFlow::empty(), false, client)
...
fn rejected_client_transaction_preserves_existing_flow_and_version() {
    ...
    let first_version = mgr
        .establish_client_flow(first_key, SocketLegFlow::empty(), true, first_client)
    ...
    assert!(
        mgr.establish_client_flow(rejected_key, SocketLegFlow::empty(), false, rejected_client,)
            .is_err()
    );
    let handles = mgr.refresh_handles();
