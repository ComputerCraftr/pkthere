...
impl SocketManager {
    ...
    pub fn establish_client_flow(
        ...
        Ok(self.publish_version(true))
    }

    /// Publish a socket-association change already completed atomically by
    /// `ManagedSocket::send_packet`.
    pub fn publish_client_association_change(&self) -> u64 {
        self.publish_version(true)
    }

    #[inline]
    pub fn clear_client_lock(&self) -> io::Result<u64> {
        let mut cl_guard = self.client_listen.lock().unwrap();
        cl_guard.flow = None;
        cl_guard.listener_flow = SocketLegFlow::empty();
        let cleared_version = self.publish_version(true);
