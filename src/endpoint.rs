    ...
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self.address {
            ...
            ScopedIp::V6 {
                ...
                flowinfo,
                scope_id,
            } => match (flowinfo, scope_id) {
                (0, 0) => write!(formatter, "[{ip}]:{}", self.id()),
                (0, scope_id) => write!(formatter, "[{ip}%{scope_id}]:{}", self.id()),
                (flowinfo, scope_id) => write!(
                    formatter,
                    "[{ip}%{scope_id};flowinfo={flowinfo}]:{}",
                    self.id()
                ),
            },
        }
...
mod tests {
    ...
    #[test]
    fn display_preserves_visible_ipv6_scope_and_flow_information() {
        assert_eq!(
            LogicalEndpoint::from_v6(Ipv6Addr::LOCALHOST, 7, 0, 0).to_string(),
            "[::1]:7"
        );
        assert_eq!(
            LogicalEndpoint::from_v6(Ipv6Addr::LOCALHOST, 7, 0, 2).to_string(),
            "[::1%2]:7"
        );
        assert_eq!(
            LogicalEndpoint::from_v6(Ipv6Addr::LOCALHOST, 7, 9, 2).to_string(),
            "[::1%2;flowinfo=9]:7"
        );
    }
}
